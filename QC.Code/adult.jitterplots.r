...
l1a.name="dt_long1.txt"
l1b.name="dt_long11.txt"
b1.name="dt_bd1.txt"   
b2.name="dt_bd2.txt"   
...
if(k==1){l1.name=l1a.name} else{l1.name=l1b.name}
...
b2<-data.frame(read.table(b2.name,sep=",",header=T))

# derive site
b2$site<-rep(NA,nrow(b2))
for(j in 1:nrow(b2)){
b2$site[j]=strsplit(as.character(b2$patient_code[j]),"\\-")[[1]][1]
}

pdf(paste(b2.name,".jitter.pdf",sep=""))

  MakePlot(as.numeric(as.character(b2$leukocyte_count)),as.numeric(as.character(b2$site)),c(0,70),"leukocyte_count",b2.name)
