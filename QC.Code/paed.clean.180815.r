...
# BARCODE - JR not specific so just checked that it the code begins with AB or ab
   ...
   if(!is.na(b1$sputum_weight[i]) && is.na(b1$resuspension_volume)){IssueOut(paste(fname,i,b1[i,1],b1[i,2],b1[i,3],"resuspension_volume","","Sputum weight but no resuspension volume\n",sep=","))}
  ...
  if(b1$sputum_sample_viability[i]!=""){
   ...
   if(!is.na(b1$serum_sample_code[i])){
...
# called consent_genetic for paeds
#    if(sc$informed_consent_genetic[i]!=""){
    if(sc$consent_genetics[i]!=""){
      if(sc$consent_genetics[i]=="no"){IssueOut(paste(fname,i,sc[i,1],sc[i,2],sc[i,3],"consent_genetics",sc$consent_genetics[i],"Consent not given: Inform Biobank\n",sep=","))}
    }
...
# following line added for paeds data
      if(!is.na(sc$hypertension_age_of_onset[i])){
      if(as.character(sc$hypertension_age_of_onset[i])=="less_than_2"){IssueOut(paste(fname,i,sc[i,1],sc[i,2],sc[i,3],"hypertension_age_of_onset",sc$hypertension_age_of_onset[i],"Less than 2\n",sep=","))}else
      if(as.character(sc$hypertension_age_of_onset[i])=="2to17"){IssueOut(paste(fname,i,sc[i,1],sc[i,2],sc[i,3],"hypertension_age_of_onset",sc$hypertension_age_of_onset[i],"Between 2 and 17\n",sep=","))}
      }
# following line added for paeds data
      if(!is.na(sc$coronary_age_of_onset[i])){
...
# following line added for paeds data
      if(!is.na(sc$congestive_age_of_onset[i])){
...
# following line added for paeds data
      if(!is.na(sc$osteoporosis_age_of_onset[i])){
 ...
 # following line added for paeds data
      if(!is.na(sc$diabetes_age_of_onset[i])){
...
# no checks specified to row 1131
...
dates<-function(long1.name,base1.name,sae.name,scre.name){ # specify all of the filenames
...
if(long1$visit_date[i]!="" && !is.na(long1$visit_date[i]) && as.character(scr.date)!=""){
lon.date<-as.Date(long1$visit_date[i],"%d/%m/%Y")
diff.date<-lon.date-scr.date
if(diff.date<0){IssueOut(paste(long1.name,i,long1[i,1],long1[i,2],long1[i,3],"visit_date",long1$visit_date[i],"Long visit date is prior to screening date\n",sep=","))}
}
...
if(diff.date<0){IssueOut(paste(long1.name,i,long1[i,1],long1[i,2],long1[i,3],"valid_spirometry_date",long1$valid_spirometry_date[i],"Spirometry date is prior to baseline date\n",sep=","))}
}
...
if(diff.date<0){IssueOut(paste(long1.name,i,long1[i,1],long1[i,2],long1[i,3],"reversibility_test_date",long1$reversibility_test_date[i],"Reversibility test date is prior to screening date\n",sep=","))}
}
...
if(diff.date<0){IssueOut(paste(base1.name,j,base1[j,1],base1[j,2],base1[j,3],"valid_spirometry_date",base1$valid_spirometry_date[j],"Spirometry date is prior to screening date\n",sep=","))}
}

if(!is.na(base1$reversibility_test_date[j]) && as.character(scr.date)!=""){
rev.date<-as.Date(base1$reversibility_test_date[j],"%d/%m/%Y")
diff.date<-rev.date-scr.date
if(diff.date<0){IssueOut(paste(base1.name,j,base1[j,1],base1[j,2],base1[j,3],"reversibility_test_date",base1$reversibility_test_date[j],"Reversibility test date is prior to screening date\n",sep=","))}
}
...
if(diff.date<0){IssueOut(paste(base1.name,j,base1[j,1],base1[j,2],base1[j,3],"plethysmography_date",base1$plethysmography_date[j],"Plethysmography date is prior to screening date\n",sep=","))}
}
...
if(diff.date<0){IssueOut(paste(base1.name,j,base1[j,1],base1[j,2],base1[j,3],"lge_sampling_date",base1$lge_sampling_date[j],"IgE sampling date is prior to screening date\n",sep=","))}
}
...
if(diff.date<0){IssueOut(paste(base1.name,j,base1[j,1],base1[j,2],base1[j,3],"skin_prick_date",base1$skin_prick_date[j],"Skin Prick date is prior to screening date\n",sep=","))}
}
...
if(diff.date<0){IssueOut(paste(base1.name,j,base1[j,1],base1[j,2],base1[j,3],"sputum_sampling_date",base1$sputum_sampling_date[j],"Sputum sampling date is prior to screening date\n",sep=","))}
}
...
if(diff.date<0){IssueOut(paste(sae.name,l,sae[l,1],sae[l,2],sae[l,3],"onset_date_event",sae$onset_date_event[l],"SAE onset date is prior to screening date\n",sep=","))}
}
...
if(diff.date<0){IssueOut(paste(sae.name,l,sae[l,1],sae[l,2],sae[l,3],"end_date_event",sae$end_date_event[l],"SAE end date is prior to screening date\n",sep=","))}
diff.date<-end.date-ons.date
if(diff.date<0){IssueOut(paste(sae.name,l,sae[l,1],sae[l,2],sae[l,3],"end_date_event",sae$end_date_event[l],"SAE end date is prior to onset date\n",sep=","))}
}
...
diff.date<-spi.date-scr.date
if(diff.date<0){IssueOut(paste(scre.name,n,scre[n,1],scre[n,2],scre[n,3],"valid_spirometry_date",scre$valid_spirometry_date[n],"Spirometry date is prior to screening date\n",sep=","))}
}
